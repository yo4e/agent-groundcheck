...
function instructionKind(path2) {
  if (path2.endsWith("/AGENTS.md") || path2 === "AGENTS.md") return "agents";
  if (path2.endsWith("/CLAUDE.md") || path2 === "CLAUDE.md") return "claude";
  if (path2.endsWith("/GEMINI.md") || path2 === "GEMINI.md") return "gemini";
  if (path2 === ".github/copilot-instructions.md") return "copilot";
  return void 0;
}
function isIgnored(path2) {
  return path2.split("/").some((segment) => IGNORED_SEGMENTS.has(segment));
}
async function discoverInstructionSources(view) {
  const files = await view.listFiles();
  const candidates = files.filter((path2) => !isIgnored(path2) && instructionKind(path2));
  const sources = [];
  for (const path2 of candidates) {
    const content = await view.readText(path2);
    const kind = instructionKind(path2);
    if (content !== null && kind) sources.push({ path: path2, kind, content });
  }
  return sources;
}
...
var GitRepoView = class {
  ...
  async exists(path2) {
    try {
      await git(this.cwd, ["cat-file", "-e", `${this.ref}:${path2}`]);
      const type = (await git(this.cwd, ["cat-file", "-t", `${this.ref}:${path2}`])).trim();
  ...
  async readText(path2) {
    try {
      ...
      const size = Number((await git(this.cwd, ["cat-file", "-s", `${this.ref}:${path2}`])).trim());
      if (!Number.isFinite(size) || size > MAX_TEXT_BYTES) return null;
      return await git(this.cwd, ["show", `${this.ref}:${path2}`]);
    } catch {
      return null;
    }
  }
