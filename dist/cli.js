...
async function discoverInstructionSources(view) {
  ...
  for (const path4 of candidates) {
    const content3 = await view.readText(path4);
    const kind = instructionKind(path4);
    if (content3 !== null && kind) sources.push({ path: path4, kind, content: content3 });
  }
  return sources;
}
...
function visit(node2, callback) {
  callback(node2);
  for (const child of node2.children ?? []) visit(child, callback);
}
function extractClaims(source) {
  const root = fromMarkdown(source.content);
  const claims = [];
  visit(root, (node2) => {
    if (node2.type === "inlineCode" && typeof node2.value === "string") {
      claims.push(...claimsFromText(source, node2.value, lineFor(node2, 1)));
    }
    if (node2.type === "code" && typeof node2.value === "string") {
      const startLine = lineFor(node2, 1) + 1;
      node2.value.split("\n").forEach((value, index2) => {
        claims.push(...claimsFromText(source, value, startLine + index2));
      });
    }
    if (node2.type === "link" && typeof node2.url === "string") {
      const pathClaim = extractPathClaim(source, node2.url, lineFor(node2, 1));
      if (pathClaim) claims.push(pathClaim);
    }
  });
  return claims;
...
function parseManifest(pathname, content3) {
  try {
    const value = JSON.parse(content3);
  ...
  const parsed = (async () => {
    ...
    const entries = await Promise.all(
      files.map(async (pathname) => {
        const content3 = await view.readText(pathname);
        return content3 === null ? void 0 : parseManifest(pathname, content3);
      })
    );
...
function parseArgs(argv) {
  ...
  const values = {};
  for (let index2 = 0; index2 < rest.length; index2 += 1) {
    const argument = rest[index2];
    if (!argument.startsWith("--")) throw new Error(`Unexpected argument: ${argument}`);
    const key = argument.slice(2);
    const value = rest[index2 + 1];
